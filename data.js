...
const JOB_SKILLS = {
  ...
  priest: [
    ...
    { id: "prayer", type: "active", name: "状態回復", description: "味方の悪い状態を治療する", effectLines: ["毒・呪い・麻痺・睡眠・火傷・スロー・盲目を回復"], requiredLevel: 6 },
...
const SKILL_DEBUG_INFO = {
  ...
  prayer: ["対象:毒", "対象:猛毒", "対象:呪い", "対象:睡眠", "対象:火傷", "対象:スロー", "対象:麻痺", "対象:盲目"],
...
const AREAS = {
  ...
  sunkenPier: {
    ...
    flavor: ["朽ちた桟橋が泥水に沈みかけ、足場は頼りない。", "浅い水面の向こうで、葦の影だけが揺れている。"],
  },
  reedWaterway: {
    id: "reedWaterway",
    name: "沈み灯の沼（葦の水路）",
    ...
    flavor: ["高い葦が視界を遮り、水路の形を曖昧にしている。", "持ち帰った地図の線が、小隊ごとに微妙にずれていた。"],
  },
  blackwaterPool: {
    id: "blackwaterPool",
    name: "沈み灯の沼（黒水の淀み）",
  ...
  ruins: {
    ...
    difficulty: 4,
    baseAtk: 14,
    monsters: ["rubbleBat", "ivySkeleton", "stoneGuard", "bellWisp"],
  ...
  outerGarden: {
    ...
    difficulty: 4,
    baseAtk: 14,
    monsters: ["rubbleBat", "ivySkeleton"],
    normalEncounterGroupWeights: { 1: 75, 2: 20, 3: 5 },
    unlockAfter: "blackwaterPool",
    trapRates: [0.75, 0.5, 0.25, 0.12],
    description: "崩れた石庭が残る遺跡の外縁。古い石片と、判読できない刻印が散らばっている。",
    flavor: ["崩れた外庭には、刻印の欠けた石片が散っていた。", "乾いた風の中で、石の欠片だけが不揃いに並んでいる。"],
  },
  crackedCorridor: {
    id: "crackedCorridor",
    name: "忘鐘の遺跡（ひび割れた回廊）",
    ...
    monsters: ["rubbleBat", "ivySkeleton", "stoneGuard", "bellWisp"],
    normalEncounterGroupWeights: { 1: 60, 2: 30, 3: 10 },
    unlockAfter: "outerGarden",
    trapRates: [0.75, 0.5, 0.25, 0.12],
    description: "壁と床に細かな亀裂が走る回廊。足音が、少し遅れて返ってくる。",
    flavor: ["回廊の亀裂は細く長く、どこまでも続いて見えた。", "足音の反響だけが、一拍遅れて壁際を戻ってきた。"],
  },
  sealedBelfry: {
    id: "sealedBelfry",
    name: "忘鐘の遺跡（鐘下の封室）",
    ...
    difficulty: 4,
    baseAtk: 14,
    monsters: ["ivySkeleton", "stoneGuard", "bellWisp"],
    normalEncounterGroupWeights: { 1: 50, 2: 35, 3: 15 },
    unlockAfter: "crackedCorridor",
    boss: "sealedBeast",
    trapRates: [0.75, 0.5, 0.25, 0.12],
    description: "失われた鐘の下にある封じられた部屋。用途の分からない破片が見つかる。",
    flavor: ["封室の床には、用途不明の破片が幾つも残されていた。", "見上げても鐘はなく、ただ重い空洞だけが続いている。"],
  },
  canyon: {
    id: "canyon",
    name: "赤鳴りの峡谷",
  ...
  windCutPass: {
    ...
    monsters: ["rockHawk"],
    normalEncounterGroupWeights: { 1: 75, 2: 20, 3: 5 },
    unlockAfter: "sealedBelfry",
    description: "赤土の崖が風に削られた峡谷の入口。乾いた音が、岩肌を渡っていく。",
    flavor: ["乾いた風が崖肌を削り、細かな砂を舞い上げていた。", "谷の入口では、空鳴りのような音だけが続いている。"],
  },
  cliffPath: {
    id: "cliffPath",
    name: "赤鳴りの峡谷（断崖の細道）",
    ...
    difficulty: 5,
    baseAtk: 18,
    monsters: ["rockHawk", "cliffLizard", "canyonBandit"],
    normalEncounterGroupWeights: { 1: 60, 2: 30, 3: 10 },
    unlockAfter: "windCutPass",
    description: "断崖沿いに続く細い道。足を滑らせれば、下の赤い谷底まで止まらない。",
    flavor: ["断崖の細道は狭く、すれ違うだけでも気を遣う。", "足元の小石が落ちるたび、赤い谷底へ音が吸い込まれた。"],
  },
  redMoonWatch: {
    id: "redMoonWatch",
    name: "赤鳴りの峡谷（赤月の見張り場）",
    ...
    normalEncounterGroupWeights: { 1: 50, 2: 35, 3: 15 },
    boss: "redGorgeWing",
    unlockAfter: "cliffPath",
    description: "崖上に残された古い見張り場。誰が使っていたのか分からない焚き火跡がある。",
    flavor: ["崖上の見張り場には、冷えた焚き火跡だけが残っていた。", "誰が使ったのか分からない矢印が、崖の奥を指している。"],
  },
  glacier: {
    id: "glacier",
    name: "白冠の氷原",
  ...
  frostMarker: {
    ...
    monsters: ["iceWolf"],
    normalEncounterGroupWeights: { 1: 70, 2: 25, 3: 5 },
    unlockAfter: "redMoonWatch",
    description: "雪に埋もれた古い道標が立つ氷原の入口。記録上の位置と、現地の位置が少し合わない。",
    flavor: ["雪中に立つ道標は、記録の写しより少し北へずれていた。", "風雪の合間に、埋もれた街道石が薄く覗いている。"],
  },
  icefield: {
    id: "icefield",
    name: "白裂の氷原（裂氷の原）",
    ...
    unlockAfter: "frostMarker",
    description: "白い氷面に長い裂け目が走る原野。風雪の中では、遠近の感覚が狂いやすい。",
    flavor: ["裂けた氷の縁が、風に鳴って細く軋んでいた。", "吹雪の奥行きが曖昧で、距離感だけが先に崩れていく。"],
  },
  fangDrift: {
    id: "fangDrift",
    name: "白裂の氷原（白牙の吹き溜まり）",
    ...
    normalEncounterGroupWeights: { 1: 35, 2: 45, 3: 20 },
    boss: "whiteFangGiant",
    unlockAfter: "icefield",
    description: "白い獣の痕跡が残る吹き溜まり。雪の下から、古い街道の名残が覗いている。",
    flavor: ["吹き溜まりには白い獣の痕跡が幾重にも残っていた。", "雪の割れ目から、古い街道石が断続的に覗いている。"],
  },
  volcano: {
    id: "volcano",
    name: "赤熱の火口",
...
const WORLD_SITUATIONS = {
  ...
  sunkenPier: [["沈み灯の沼入口の桟橋が、さらに傾いたと報告された。"]],
  reedWaterway: [["葦の水路では、小隊ごとに道順の記録が少し食い違っている。"]],
  blackwaterPool: [["黒水の淀みで、水面下の影を見たという報告が続いている。"]],
  ruins: [["忘れられた遺跡内部で未知の紋章が発見された。", "調査班は夜間活動を停止した。"]],
  outerGarden: [["忘鐘の遺跡の外庭で、判読不能の刻印片が複数回収された。"]],
  crackedCorridor: [["ひび割れた回廊では、足音の反響が遅れる現象が続いている。"]],
  sealedBelfry: [["鐘下の封室から、用途不明の破片が持ち帰られた。"]],
  canyon: [["赤鳴りの峡谷監視隊より、説明できない落石音の報告あり。", "吊り橋の通行許可はギルド管理下へ移された。"]],
  windCutPass: [["峡谷入口では、乾いた風鳴りが夜まで止まない。"]],
  cliffPath: [["断崖の細道で、荷運び隊の足跡が途中で途切れていた。"]],
  redMoonWatch: [["赤月の見張り場で、誰のものか分からない焚き火跡が見つかった。"]],
  glacier: [["白冠の氷原方面の監視塔が一晩で凍結した。", "防寒符の追加配布を受付で開始した。"]],
  frostMarker: [["凍て道の標は、記録上の位置と少しずれて見つかった。"]],
  icefield: [["裂氷の原では、吹雪の中で距離感が狂うという報告が増えている。"]],
  fangDrift: [["白牙の吹き溜まりで、雪下の古い街道石が確認された。"]],
...
const MONSTERS = {
  ...
  forestWarden: { id: "forestWarden", name: "フォレストファング", hp: 62, atk: 14, xp: 46, gold: 52, rare: true, boss: true, special: "biteCrushLite", traitsText: "森の奥に棲む大牙。噛み砕きと威嚇で獲物を追い詰める。", drops: ["blackLeatherArmor"] },
  ...
  frostWisp: { id: "frostWisp", name: "フロストウィスプ", hp: 58, atk: 22, xp: 58, gold: 52, rare: true, special: "frostGlimmerLite", traitsText: "冷たい光で道を惑わせる灯霊。", drops: ["oldCharm"] },
