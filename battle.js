...
function applyOutgoingDamagePenalty(actor, damage) {
  if ((actor?.[STATUS_EFFECTS.atkDown.turnKey] || 0) <= 0) return damage;
...
function tickMemberTurnStatuses(member, events = null) {
  ...
  if (member?.[STATUS_EFFECTS.atkDown.turnKey] > 0) {
    member[STATUS_EFFECTS.atkDown.turnKey] -= 1;
    if (member[STATUS_EFFECTS.atkDown.turnKey] <= 0) {
      member[STATUS_EFFECTS.atkDown.turnKey] = 0;
    }
  }
...
function performEnemyAction(enemy, party, enemies, area, heroLevel, events, speechState, round = 1) {
  ...
  if (enemy.special === "strongSlashLite" && Math.random() < 0.5) {
    ...
    const canUsePriestBlessing =
      ...
      target.hp > 0;
    applyDamageToMember(target, damage);
    events.push({ kind: "enemy-action", text: `${enemy.name}の強斬り。` });
    if (cover.coverer) {
      if (Math.random() < 0.15) {
        events.push({ kind: "enemy-action", text: `${cover.coverer.name}「受け止める！」` });
      }
      events.push({ kind: "enemy-action", text: `${cover.coverer.name}が${cover.covered.name}をかばった。` });
    }
    if (target.hp <= 0) {
  ...
  if (enemy.special === "biteCrushLite" && Math.random() < 0.5) {
    ...
    if (intimidateTargets.length && Math.random() >= 0.7) {
      target = pick(intimidateTargets);
      target[STATUS_EFFECTS.atkDown.turnKey] = 2;
      events.push({ kind: "enemy-action", text: `${enemy.name}の威嚇。` });
      events.push({ kind: "enemy-action", text: `${target.name}はたじろぎ、攻め手が鈍った。` });
      pushHp(events, target);
      tickEnemyDots(enemy, events);
      tickEnemyTurnStatuses(enemy);
      return;
    ...
    const canUsePriestBlessing =
      ...
      target.hp > 0;
    applyDamageToMember(target, damage);
    events.push({ kind: "enemy-action", text: `${enemy.name}の噛み砕き。` });
    if (cover.coverer) {
      if (Math.random() < 0.15) {
        events.push({ kind: "enemy-action", text: `${cover.coverer.name}「こっちだ！」` });
      }
      events.push({ kind: "enemy-action", text: `${cover.coverer.name}が${cover.covered.name}をかばった。` });
    }
    if (target.hp <= 0) {
  ...
  if (enemy.special === "frostGlimmerLite" && Math.random() < 0.6) {
    const predictedDamage = Math.max(1, Math.floor(damageFor(enemy.atk, target.def) * 0.6));
    const cover = maybeCoverTarget(party, target, predictedDamage);
    target = cover.target;
    ...
    const canUsePriestBlessing =
      ...
      target.hp > 0;
    applyDamageToMember(target, damage);
    events.push({ kind: "enemy-action", text: `${enemy.name}が冷たい光を揺らした。` });
    if (cover.coverer) {
      if (Math.random() < 0.15) {
        events.push({ kind: "enemy-action", text: `${cover.coverer.name}「下がって！」` });
      }
